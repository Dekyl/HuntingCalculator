from PySide6.QtWidgets import QWidget, QVBoxLayout, QLabel, QLineEdit, QHBoxLayout, QCheckBox, QComboBox, QPushButton
...
class SettingsWidget(QWidget):
    def __init__(self):
        ...
        for setting_name, setting_val in self.original_settings.items():
            ...
            if text_setting_label in {'Show Confirm Clean Sessions Message', 'Show Confirm Exit Message', 'Value Pack', 'Extra profit (Ring, Old moon...)'}:
                    check_box = QCheckBox()
                ...
                if text_setting_label == 'Region':
                    combo_box.addItems(['eu', 'na', 'asia'])
                    combo_box.setCurrentText(setting_val)
                elif text_setting_label == 'Language':
                    combo_box.addItems(['en-US', 'fr-FR', 'de-DE', 'es-ES'])
                    combo_box.setCurrentText(setting_val)
                ...
                setting_layout.addWidget(setting_line_edit, 0, Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
